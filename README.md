...
- “How many case studies are in Google Drive?” counts documents under `Docs/Case Studies/` (or another folder named `Case Studies`) and its subfolders, independent of filename. `Dream.pdf` is included if it lives there; a filename containing “case study” outside that folder does not establish the category. Drive sync stores root-relative `folderPath` and `category` metadata and includes the folder hierarchy in citation paths. Folder moves and renames refresh metadata for unchanged documents without re-embedding.
